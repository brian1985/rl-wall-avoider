...
from config_ResizeAndMove import environ_settings as settings
...
######################################################################################
class Environment:
    P_HEIGHT = settings['P_HEIGHT']  # Height of the player
    F_HEIGHT = settings['F_HEIGHT']  # Height of the field
    W_HEIGHT = settings['W_HEIGHT']  # Height of the walls
    WIDTH = settings['WIDTH'] # Width of the field and the walls
    MIN_H_WIDTH = settings['MIN_H_WIDTH']  # Minimum width of the holes
    MAX_H_WIDTH = settings['MAX_H_WIDTH']  # Maximum width of the holes
    MIN_P_WIDTH = settings['MAX_P_WIDTH']  # Minimum Width of the player
    MAX_P_WIDTH = settings['MAX_P_WIDTH']  # Maximum Width of the player
    HEIGHT_MUL = settings['HEIGHT_MUL']  # Height Multiplier (used to draw np.array as blocks in pygame )
    WIDTH_MUL = settings['WIDTH_MUL']  # Width Multiplier (used to draw np.array as blocks in pygame )
    WINDOW_HEIGHT = settings['WINDOW_HEIGHT']  # Height of the pygame window
    WINDOW_WIDTH = settings['WINDOW_WIDTH']  # Widh of the pygame window

    ENVIRONMENT_SHAPE = settings['ENVIRONMENT_SHAPE']
    ACTION_SPACE = settings['ACTION_SPACE']
    ACTION_SPACE_SIZE = len(ACTION_SPACE)
    PUNISHMENT = settings['PUNISHMENT']  # Punishment increment
    REWARD = settings['REWARD']  # Reward increment
    score = settings['score']  # Initial Score

    MOVE_WALL_EVERY = settings['MOVE_WALL_EVERY']  # Every how many frames the wall moves.
    MOVE_PLAYER_EVERY = settings['MOVE_PLAYER_EVERY']  # Every how many frames the player moves.
    frames_counter = settings['frames_counter']

    COMPUTER_PLAYER = settings['COMPUTER_PLAYER']
    def __init__(self):
        # Colors:
        self.BLACK = (25, 25, 25)
    ...
    def reset(self):
        ...
        w1 = Wall(height=self.W_HEIGHT, width=self.WIDTH,
                  hole_width=4,  # randint(self.MIN_H_WIDTH,self.MAX_H_WIDTH),
                  field=self.field)
        self.walls = deque([w1])
        p_width = 4  # randint(self.MIN_P_WIDTH,self.MAX_P_WIDTH)
    ...
    def step(self, action):
        ...
        if action in [1, 5, 6]:
            self.player.move(direction=self.player.MOVE_LEFT, field=self.field)
        # If the performed action is (change_width) then player.change_width method is called:
        if action in [2, 7, 8]:
            self.player.move(direction=self.player.MOVE_RIGHT, field=self.field)
        if action in [3, 5, 7]:
            self.player.change_width(action=self.player.SHRINK_SIZE)
        if action in [4, 6, 8]:
            self.player.change_width(action=self.player.EXPAND_SIZE)
...
class ModifiedTensorBoard(TensorBoard):
    ...
    def __init__(self, name, **kwargs):
        ...
        self.step = 1
        self.writer = tf.summary.create_file_writer(self.log_dir+ "/metrics")
...
class DQNAgent:
    def __init__(self, name, env, conv_list, dense_list, util_list, use_existing_weights=False):
        self.env = env
        ...
        # used to init weights from a previous training - Set before call to create_model
        self.use_existing_model = use_existing_weights
    ...
    def init_weights_existing_model(self, name, model):
        """
        Should only be called after a model has been defined in the code. Model must mast in
        architecture..
        :return: model
        """
        from keras.models import load_model
        print("weights being init")
        model_name_ = "M0_32C_16C_32D_32D_ECC2_1A-5Ac__Step3.model"
        old_model = load_model(f'models/{model_name_}')
        old_model.get_weights()
        model.set_weights(old_model.get_weights())

        return model

    def conv_block(self, inp, filters=32, bn=True, pool=True, dropout=0.2):
    ...
    def create_model(self, conv_list, dense_list):
        ...
        _ = self.conv_block(input_layer, filters=conv_list[0], bn=False, pool=True)
        ...
        if self.use_existing_model:
            model = self.init_weights_existing_model(None, model)
...
def save_model_and_weights(agent, model_name, episode, max_reward, average_reward, min_reward):
    checkpoint_name = f"{model_name}_Eps({episode})_max({max_reward:_>7.2f})_avg({average_reward:_>7.2f})_min({min_reward:_>7.2f}).model"
...
if __name__ == "__main__":
    ...
    SAVE_MODEL_EVERY = 50  # Episodes
    SHOW_EVERY = 100  # Episodes
    ...
    models_arch = [{"conv_list": [32], "dense_list": [32, 32], "util_list": ["ECC2", "1A-5Ac"],
                   ...
                   {"conv_list": [32, 16], "dense_list": [32, 32], "util_list": ["ECC2", "1A-5Ac"],
                    "MINIBATCH_SIZE": 512, "best_only": False,
    ...
    # Grid Search:
    if True:
        # manual code to run if desired
        models_arch = [models_arch[2]]
        i = 0
    ...
    for i, m in enumerate(models_arch):
        ...
        MAX_EPSILON = .95  # Maximum epsilon value
        MIN_EPSILON = 0.03  # Minimum epsilon value
        if EF_Enabled:
            ...
            epsilon = .95  # not a constant, going to be decayed
        else:
        ...
        max_steps = []
        env = Environment()
        env.MOVE_WALL_EVERY = settings['MOVE_WALL_EVERY']  # Every how many frames the wall moves.

        agent = DQNAgent(f"M{i}", env, m["conv_list"], m["dense_list"], m["util_list"], use_existing_weights=False)
        ...
        for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
            ...
            max_steps.append(step-1)
            if not episode % AGGREGATE_STATS_EVERY:
                print("Last 20 Episode Rewards:", ep_rewards[-20:])
                average_reward = sum(ep_rewards[-AGGREGATE_STATS_EVERY:]) / len(ep_rewards[-AGGREGATE_STATS_EVERY:])
                min_reward = min(ep_rewards[-AGGREGATE_STATS_EVERY:])
                max_reward = max(ep_rewards[-AGGREGATE_STATS_EVERY:])
                agent.tensorboard.update_stats(reward_avg=average_reward, max_step=max_steps[-1], reward_max=max_reward, epsilon=epsilon)
    ...
    print("View tensorboard in terminal. From root dir, Run: tensorboard --logdir=logs/ ::Can be used during training")
    ######################################################################################
